...
lua_cjson_tar_path = ::File.join(
  ::Chef::Config['file_cache_path'],
  "lua-cjson-#{node[id]['version']}.tar.gz"
)
lua_cjson_src_url = "#{node[id]['url']}/#{node[id]['version']}.tar.gz"
lua_cjson_src_dir = ::File.join(
  ::Chef::Config['file_cache_path'],
  "lua-cjson-#{node[id]['version']}"
)
...
execute "tar --no-same-owner -zxf #{::File.basename(lua_cjson_tar_path)} -C "\
        "#{lua_cjson_src_dir} --strip-components 1" do
